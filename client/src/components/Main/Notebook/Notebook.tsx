...
const Notebook = () => {
    ...
    useEffect(() => {
        async function getNotebookText() {
            const text: AxiosResponse<string> = await axios.get("http://localhost:5000/api/notebook/get", {headers: {Authorization: `Bearer ${localStorage.getItem("token")}`}});
            setValue(text.data);
            setIsLoading(false);
        }
        getNotebookText();
    ...
    useEffect(() => {
        ...
        setTimer(setTimeout(() => {
                axios({
                    method: "put",
                    url: "http://localhost:5000/api/notebook/set",
